...
<head>
    <meta charset="utf-8">
    <title>Shreeja Finance - Customer Repository</title>
    <style>
        ...
        .cst-id { font-size: 8px; font-weight: bold; color: #0346cb; background: #eef2ff; padding: 3px 8px; border-radius: 6px; display: inline-block; margin-top: 5px; text-transform: uppercase; }
        .status-badge { font-size: 8px; font-weight: bold; padding: 4px 8px; border-radius: 6px; text-transform: uppercase; border: 1px solid #dcfce7; background: #f0fdf4; color: #166534; }
    </style>
</head>
<body>
    <header>
        <span class="company-name">Shreeja Finance</span>
        <span style="float: right;" class="document-title">CUSTOMER REPOSITORY DIRECTORY</span>
    </header>

    <footer>
        Shreeja Finance Customer Analytics - Generated: {{ now()->format('d M, Y h:i A') }} - Internal Data Integrity
    </footer>
    ...
    <main>
        <div class="summary-box">
            <div class="summary-item">
                ...
                <div class="summary-value">{{ count($customers) }}</div>
            </div>
            <div class="summary-item">
        ...
        <table class="table">
            ...
            <tbody>
                @foreach($customers as $customer)
                    <tr>
                        <td>
                            <div style="font-weight: bold; font-size: 11px;">{{ $customer->name }}</div>
                            <div class="cst-id">CST-{{ sprintf('%04d', $customer->id) }}</div>
                        </td>
                        <td>
                            <div style="font-weight: bold;">{{ $customer->phone }}</div>
                        </td>
                        <td>
                            <div style="font-size: 9px; color: #475569;">{{ $customer->email }}</div>
                        </td>
                        <td>
                            <span class="status-badge">ACTIVE</span>
                        </td>
                        <td>
                            <div style="font-weight: bold;">{{ $customer->created_at ? $customer->created_at->format('d M, Y') : 'N/A' }}</div>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        <div style="margin-top: 50px; border-top: 1px dashed #cbd5e1; padding-top: 25px; font-size: 9px; color: #64748b; text-align: justify; line-height: 1.6;">
            <strong>Confidentiality Notice:</strong> This Customer Repository contains sensitive personal identification information belonging to Shreeja Finance clients. Unauthorized distribution, copying, or disclosure of the contents within this report is strictly prohibited and may result in legal consequences. This document serves as an official internal record for customer database management as of the generation timestamp.
        </div>
    </main>
</body>
</html>
