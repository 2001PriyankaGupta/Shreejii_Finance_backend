        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-r from-[#0346cbff] to-indigo-800 rounded-[2.5rem] p-10 mb-10 shadow-[0_20px_50px_-15px_rgba(3,70,203,0.3)] text-white relative overflow-hidden group">
                <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <h3 class="text-4xl font-black mb-2 tracking-tight">Employee Directory</h3>
                        <p class="text-blue-100 font-medium opacity-90">Manage your workforce, roles, and documents efficiently.</p>
                    </div>
                    <a href="{{ route('admin.employees.create') }}" class="inline-flex items-center px-8 py-4 bg-white text-[#0346cbff] font-black rounded-2xl transition-all shadow-xl hover:scale-105 active:scale-95 group-hover:shadow-white/20">
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl border border-white p-8 overflow-hidden group hover:border-[#0346cbff]/30 transition-all duration-500">
                <div class="mb-8 flex items-center justify-between">
                    <div>
                        <h4 class="text-2xl font-black text-slate-900 tracking-tight">Active Employees</h4>
                        <p class="text-slate-400 text-sm font-medium mt-1 uppercase tracking-widest text-[10px]">Total {{ $employees->count() }} members registered</p>
                    </div>
                </div>
                        ...
                        <tbody class="divide-y divide-transparent">
                            @forelse($employees as $employee)
                                <tr class="bg-slate-50/50 hover:bg-white hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-300 rounded-[2rem] group/row scale-100 hover:scale-[1.01]">
                                    <td class="px-6 py-6 rounded-l-[1.5rem]">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-5">
                                                <div class="font-black text-slate-900 text-base leading-tight">{{ $employee->name }}</div>
                                                <div class="text-[10px] font-black text-[#0346cbff] bg-[#0346cbff]/5 px-2 py-0.5 rounded-md mt-1 inline-block uppercase tracking-widest">ID: {{ $employee->employee_id ?? 'N/A' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-6">
                                        <div class="text-slate-600 font-bold">{{ $employee->email }}</div>
                                        <div class="text-[11px] font-black text-slate-400 tracking-wider">{{ $employee->phone }}</div>
                                    </td>
                                    <td class="px-6 py-6">
                                        <span class="px-4 py-1.5 bg-indigo-500/10 text-indigo-600 rounded-xl text-[10px] font-black uppercase tracking-widest border border-indigo-500/10 mb-1 inline-block">
                                            {{ $employee->employeeDetail?->designation ?? 'UNASSIGNED' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-6 text-slate-500 font-bold">
                                        <div class="flex items-center">
                                    ...
                                    <td class="px-6 py-6 rounded-r-[1.5rem] text-right">
                                        <div class="flex justify-end space-x-2">
                                            <a href="{{ route('admin.employees.show', $employee->id) }}" class="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-[#0346cbff] hover:text-white transition-all shadow-sm">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                            </a>
                                            <a href="{{ route('admin.employees.edit', $employee->id) }}" class="w-10 h-10 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center hover:bg-amber-500 hover:text-white transition-all shadow-sm">
