        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl border border-white p-8 overflow-hidden group hover:border-[#0346cbff]/30 transition-all duration-500">
                ...
                <div class="overflow-x-auto">
                    <table id="customersTable" class="w-full text-sm text-left border-separate border-spacing-y-4">
                        <thead>
                            <tr class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">
                                <th class="px-6 py-4 whitespace-nowrap">Customer Identity</th>
                                ...
                                <tr class="bg-slate-50/50 hover:bg-white hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-300 rounded-[2rem] group/row scale-100 hover:scale-[1.01]">
                                    <td >
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-5">
                                                <div class="font-black text-slate-900 text-base leading-tight">{{ $customer->name }}</div>
                                                <div class="text-[10px] font-black text-[#0346cbff] bg-[#0346cbff]/5 px-2 py-0.5 rounded-md mt-1 inline-block uppercase tracking-widest">ID: CST-{{ sprintf('%04d', $customer->id) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td >
                                    ...
                                    <td class=" rounded-r-[1.5rem] text-right">
                                        <div class="flex justify-end space-x-2">
                                            <a href="{{ route('admin.customers.show', $customer->id) }}" class="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-[#0346cbff] hover:text-white transition-all shadow-sm">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#customersTable').DataTable({
                ...
                buttons: [
                    { extend: 'copy', text: '<i class="fas fa-copy mr-1"></i> Copy' },
                    { extend: 'excel', text: '<i class="fas fa-file-excel mr-1"></i> Excel' },
                    { extend: 'pdf', text: '<i class="fas fa-file-pdf mr-1"></i> PDF' },
